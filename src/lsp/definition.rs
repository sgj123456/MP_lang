...
impl MpDefinition {
    ...
    pub fn goto_definition(
        ...
        if let Some(symbol_infos) = symbols.get(&target_name) {
            if let Some(first) = symbol_infos.first() {
    ...
    fn find_identifier_at(&self, tokens: &[crate::lexer::Token], line: usize, col: usize) -> Option<String> {
        for token in tokens {
    ...
    fn find_token_by_name(&self, name: &str, tokens: &[crate::lexer::Token]) -> Option<crate::lexer::Token> {
        for token in tokens {
